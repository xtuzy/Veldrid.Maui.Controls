...
{
    public class Camera_LookAt : BaseGpuDrawable
    {
...
void main()
{
    FragColor = mix(texture(sampler2D(Texture1, Sampler), TexCoord),texture(sampler2D(Texture2, Sampler),TexCoord) ,0.2);
        ...
        protected override void Draw(float deltaSeconds)
        {
            ...
            float radius = 10.0f;
            float camX =MathF.Sin(_ticks/1000) * radius;
            float camZ = MathF.Cos(_ticks/1000) * radius;
            var view = Matrix4x4.CreateLookAt(new Vector3(camX, 0.0f, camZ), Vector3.Zero, Vector3.UnitY);
